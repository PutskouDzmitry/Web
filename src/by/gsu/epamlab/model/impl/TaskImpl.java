...
public class TaskImpl implements TaskDAO{
	...
	@Override
	public void operationTask(String login, Map<String, String[]> parameterMap) throws DaoException, ValidationException {
		try {
			Operations operation = Operations.valueOf(parameterMap.get(ConstantsJSP.TYPE_OPER_KEY)[0].toUpperCase());
			String query = operation.getSqlStr();
			operation.getActFunction().getTask(login, parameterMap, query);
		}
		catch (SQLException e) {
			throw new DaoException(ConstantsSQL.ERROR_WITH_DATABASE, e);
		}
